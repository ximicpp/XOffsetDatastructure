...
XOffsetDatastructure is a **single-header C++26 library** (`xoffsetdatastructure.hpp`, ~1480 lines) for zero-copy serializable data structures using `offset_ptr`-based memory. It requires **Clang with P2996 reflection support** (a custom Clang fork) — not standard Clang or GCC.

Key concepts: offset-pointer containers (XString, XVector, XSet, XMap), compile-time type safety via Domain S admission, reflection-based zero-boilerplate construction, and memory compaction.
...
```bash
./build.sh                    # auto-detects P2996 Clang, builds + runs all 23 tests
./build.sh --no-reflection    # basic 7 tests only (no P2996 required)
...
- **Reflection (16)**: test_reflection_core, test_reflection_advanced, test_type_signatures, test_field_limit_fix, test_type_safety, test_enum_support, test_xstring_direct_assign, test_xhandle, test_error_paths, test_memory_efficiency, test_zero_boilerplate, test_zero_boilerplate_vector, test_complex_nesting, test_inheritance, test_adaptive_reservation, test_policy_trait
